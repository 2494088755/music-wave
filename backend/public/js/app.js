 ...
 return `
  <div class="card" onclick="openPlaylistDetail(${pl.id})">
   <img class="card-image" loading="lazy"src="${coverUrl}?param=200y200" 
      onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23282828%22 width=%22200%22 height=%22200%22/%3E%3Cpath d=%22M85 64v80a16 16 0 1 0 20 20V64l50-8v62a16 16 0 1 0 20 20V48L85 64z%22 fill=%22%23727272%22 opacity=%220.6%22/%3E%3C/svg%3E'"
      alt="${escapeHtml(pl.name)}">
   <div class="card-title">${escapeHtml(pl.name)}</div>
   <div class="card-subtitle">${pl.trackCount || 0} 首</div>
   <button class="card-play-btn" onclick="event.stopPropagation();openPlaylistDetail(${pl.id})">${icon("play",16)}</button>
  </div>
 `;
}
...
function createToplistCard(pl) {
 ...
 return `
  <div class="card" onclick="openPlaylistDetail(${pl.id})">
   <img class="card-image" loading="lazy"src="${coverUrl}?param=200y200" 
      onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23282828%22 width=%22200%22 height=%22200%22/%3E%3Cpath d=%22M70 40h60v20a30 30 0 0 0 30 30 30 30 0 0 0 30-30V40h20%22 stroke=%22%23727272%22 stroke-width=%224%22 fill=%22none%22/%3E%3Cpath d=%22M100 90v30%22 stroke=%22%23727272%22 stroke-width=%224%22 fill=%22none%22/%3E%3Cpath d=%22M70 120h60%22 stroke=%22%23727272%22 stroke-width=%224%22 fill=%22none%22/%3E%3C/svg%3E'"
      alt="${escapeHtml(pl.name)}">
   <div class="card-title">${escapeHtml(pl.name)}</div>
  ...
  let html = `
   <div class="playlist-detail-header">
    <img class="playlist-detail-cover" src="${coverUrl}?param=300y300" 
       onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22300%22 height=%22300%22%3E%3Crect fill=%22%23282828%22 width=%22300%22 height=%22300%22/%3E%3Cpath d=%22M120 84v120a24 24 0 1 0 30 30V84l75-12v93a24 24 0 1 0 30 30V60L120 84z%22 fill=%22%23727272%22 opacity=%220.6%22/%3E%3C/svg%3E'"
       alt="${escapeHtml(name)}">
...
async function syncNeteasePlaylists() {
 ...
 try {
  ...
  Player.showToast(' 同步失败: ' + error.message);
 } finally {
  btn.disabled = false;
...
async function confirmGuestCookie() {
 ...
 if (!cookie || cookie === '我已经设置过 Cookie，点击「清除」可重新设置') {
  Player.showToast(' 请粘贴你的 MUSIC_U cookie');
  return;
 }
 ...
 try {
  ...
  Player.showToast(' Cookie 保存失败: ' + error.message);
 }
}
...
async function clearGuestCookie() {
 try {
  ...
  Player.showToast(' 游客 Cookie 已清除');
  document.getElementById('guestCookieInput').value = '';
  checkGuestCookieStatus();
  closeGuestCookieModal();
 } catch (error) {
  Player.showToast(' 清除失败: ' + error.message);
 }
}
...
async function addCurrentSongToPlaylist(playlistId) {
 ...
 try {
  ...
  if (result.code === 200) {
   ...
   Player.showToast(' ' + (result.msg || '添加失败'));
  }
 } catch (e) {
  Player.showToast(' 添加失败');
 }
}
...
async function openLocalPlaylist(playlistId) {
 ...
 try {
  ...
  container.innerHTML = `<div class="empty-state"><div class="empty-icon"></div><div class="empty-text">加载失败: ${e.message}</div></div>`;
 }
}
...
// Sleep timer: listen for 'sleepTimerStop' event instead of monkey-patching onEnded
Player.on('sleepTimerStop', () => {
...
// Desktop notifications on song change (when tab is in background)
if ('Notification' in window && Notification.permission === 'default') {
 Notification.requestPermission();
}
Player.on('songchange', (song) => {
 if ('Notification' in window && Notification.permission === 'granted' && document.hidden && song && song.name) {
  const n = new Notification(song.name, {
   body: song.artist || '',
   icon: song.cover || '/icons/icon-192.svg',
   tag: 'musicwave-song',
  });
...
function loadLikedSongsView() {
 ...
 container.innerHTML = `
  <div class="content-header">
   <h2 class="content-title"> 我喜欢 <span style="font-size:14px;font-weight:400;color:var(--text-tertiary);">${liked.length} 首</span></h2>
  </div>
